...
public class ContainerWickerBasket extends ContainerInvTileEntity<TileEntityWickerBasket> {
	public ContainerWickerBasket(EntityPlayer player, TileEntityWickerBasket tileEntityIn) {
		...
		for (int x = 0; x < 9; ++x) {
			addSlotToContainer(
					new SlotConfigurableAccess(itemHandler, x, x * 18 + 8, 18).setCanExtract(true).setCanInsert(true));
		}
		...
		for (i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				addSlotToContainer(new SlotConfigurableAccess(playerInv, j + i * 9 + 9, 8 + j * 18, 50 + i * 18)
						.setCanExtract(true).setCanInsert(true));
			}
		}
		// Player Hotbar
		for (i = 0; i < 9; ++i) {
			addSlotToContainer(
					new SlotConfigurableAccess(playerInv, i, 8 + i * 18, 108).setCanExtract(true).setCanInsert(true));
		}
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index < TileEntityWickerBasket.INV_SIZE) {
				if (!this.mergeItemStack(containedStack, TileEntityWickerBasket.INV_SIZE, this.inventorySlots.size(),
						true)) {
