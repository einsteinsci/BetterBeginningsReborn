...
public class BlockEnderSmelter extends BlockSpecializedFurnace {
	...
	public static void updateBlockState(boolean lit, World world, BlockPos pos) {
		...
		if (lit) {
			world.setBlockState(pos,
					RegisterBlocks.enderSmelterLit.getDefaultState().withProperty(FACING, state.getValue(FACING)), 3);
		} else {
			world.setBlockState(pos,
					RegisterBlocks.enderSmelter.getDefaultState().withProperty(FACING, state.getValue(FACING)), 3);
		}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		player.openGui(ModMain.modInstance, BBGuiHandler.ENDERSMELTER_ID, world, pos.getX(), pos.getY(), pos.getZ());
