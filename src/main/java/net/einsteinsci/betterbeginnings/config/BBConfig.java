...
public class BBConfig
{
	...
	public static boolean autoRegenJsons;

	public static boolean forceAdvancedCrafting;
	...
	public static void syncConfig(Configuration config)
	{
		...
		autoRegenJsons = config.getBoolean("Auto Regen JSONs", GENERAL, true, "Regenerate main JSON recipe files if new recipes are added to vanilla BB");
		...
		moduleAdvancedCrafting = config.getBoolean("Module - Advanced crafting", GENERAL, true,
			"Enable BetterBeginnings Advanced Crafting recipes. Vanilla recipes will only be removed if forceAdvancedCrafting is true. Requires deleting Advanced Crafting recipe main JSON to have an effect");
		...
		// Crafting
		forceAdvancedCrafting = config.getBoolean("Force Advanced crafting", GENERAL, true,
	...
	public static void fillAlsoPickaxes()
	{
		...
		if(!alsoPickaxesStrings.isEmpty()) LogUtil.log(Level.INFO, "Loading pickaxes from config");
		for (String entry : alsoPickaxesStrings)
		{
			...
			String regName = entry.substring(0, equalsAt);
			Item item = RegistryUtil.getItemFromRegistry(regName);
			if (item == null)
			{
			    	LogUtil.log(Level.ERROR, "No item found with name '" + regName + "'.");
				continue;
			}
	...
	public static void fillAlsoAxes()
	{
		...
		if(!alsoAxesStrings.isEmpty()) LogUtil.log(Level.INFO, "Loading axes from config");
